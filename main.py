    ...
    def label_spravka_fill(self):
        ...
        if zadacha_type == "Тестовая":
            ...
            text = (
                ...
                "------------------------------------------------------------------<br>"
                f"Для решения основной задачи использована сетка с числом разбиений "
                f"по x <b>n = {self.res_n}</b> и по y <b>m = {self.res_m}</b>. <br>"
                f"Метод: <b>Минимальных невязок (ММН)</b>. <br>"
                f"Критерии остановки по точности <b>ε<sub>мет</sub> = {self.e_max_1}</b> "
                f"и по числу итераций <b>N<sub>max</sub> = {self.n_max_1}</b>. <br>"
                f"На решение СЛАУ затрачено итераций <b>N = {self.res_iter}</b>, "
                f"достигнута точность итерационного метода <b>ε(N) = {self.res_eps_n}</b>. <br>"
                f"Схема (СЛАУ) решена с невязкой <b>||R(N)|| = {self.res_r_n}</b>. <br>"
                f"Использована <b>норма Чебышёва</b>. <br>"
                f"В качестве начального приближения на основной сетке использовано: <b>нулевое</b>. <br>"
                f"Начальная невязка СЛАУ <b>||R(0)|| = {self.res_r_0}</b> (норма Чебышёва). <br>"
                f"Время расчета на основной сетке: <b>{t1:.4f} сек.</b><br>"
                "------------------------------------------------------------------<br>"
                f"Для контроля точности использована сетка с половинным шагом "
                f"(<b>{int(self.res_n) * 2}x{int(self.res_m) * 2}</b>). <br>"
                f"Критерии остановки по точности <b>ε<sub>мет-2</sub> = {self.e_max_2}</b> "
                f"и по числу итераций <b>N<sub>max-2</sub> = {self.n_max_2}</b>. <br>"
                f"На решение СЛАУ затрачено итераций <b>N2 = {self.res_iter_2}</b>, "
                f"достигнута точность метода <b>ε(N2) = {self.res_eps_n_2}</b>. <br>"
                f"Схема на сетке с половинным шагом решена с невязкой <b>||R(N2)|| = {self.res_r_n_2}</b>. <br>"
                f"Использована <b>норма Чебышёва</b>. <br>"
                f"В качестве начального приближения на сетке с половинным шагом использовано: <b>нулевое</b>. <br>"
                f"На сетке с половинным шагом начальная невязка <b>||R(0)|| = {self.res_r_0_2}</b>. <br>"
                f"Время расчета на сетке с пол. шагом: <b>{t2:.4f} сек.</b><br>"
                "------------------------------------------------------------------<br>"
                f"Задача должна быть решена с точностью не хуже <b>ε = 0.5·10⁻⁶</b>. <br>"
                f"Задача решена с точностью <b>ε2 = {self.res_error}</b>. <br>"
                f"Максимальное отклонение решений в узле: <b>x = {self.res_x_max}</b>; <b>y = {self.res_y_max}</b>. <br>"

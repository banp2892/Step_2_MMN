...
double DirihleVPuassone::solve() {
	...
	if (std::abs(ar_ar) < 1e-16) return 0.0; // проверка делени€ на ноль
...
void DirihleVPuassone::print_final_accuracy() {
	...
	for (int j = 0; j <= m; j++) {
		...
		for (int i = 0; i <= n; i++) {
			double x = a + i * h;
			double exact = sin(x * y * y) * sin(x * y * y); // “вое u_test
			double diff = std::abs(v[j * (n + 1) + i] - exact);
			if (diff > max_diff) max_diff = diff;
		}
	}
	std::cout << "ћаксимальное отклонение от точного решени€: " << max_diff << std::endl;
}